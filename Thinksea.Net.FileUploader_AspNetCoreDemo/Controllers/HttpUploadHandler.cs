...
namespace Thinksea.Net.FileUploader_AspNetCoreDemo.Controllers
{
    /// <summary>
    /// 上传文件服务。
    /// </summary>
    [ResponseCache(Duration = 0, Location = ResponseCacheLocation.None, NoStore = true)]
    //[Microsoft.AspNetCore.Cors.EnableCors()] //跨域
    //[IgnoreAntiforgeryToken(Order = 1001)] //禁用此页面的防伪令牌验证。（ValidateAntiForgeryToken标记默认的Order属性为1000，因此IgnoreAntiforgeryToken属性需要一个更高的序号）
    [ApiController]
    [Route("[controller]")]
    public class HttpUploadHandler: ControllerBase
    {
        private readonly ILogger<HttpUploadHandler> _logger;

        /// <summary>
        /// 用指定的数据初始化此实例。
        /// </summary>
        /// <param name="logger">日志提供程序。</param>
        public HttpUploadHandler(ILogger<HttpUploadHandler> logger)
        {
            _logger = logger;
        }
        ...
        /// <summary>
        /// 当页面以 HTTP POST 方式加载时调用此方法。
        /// </summary>
        [HttpPost]
        public void Post()
        {
            customHttpUploadServer.ProcessRequest(this.HttpContext);
        }
    ...
    {
        public new void ProcessRequest(Microsoft.AspNetCore.Http.HttpContext context)
        {
